...
def test_one(filedir, posQuantscale, resolution, test_psnr=False):
    ...
    log_enc = gpcc_encode(filedir, bin_dir, posQuantscale=posQuantscale, 
                        tmc3dir='tmc3_v14', cfgdir='dense.cfg')
...
def test_all(filedir, posQuantscaleList, resolution, test_d2=False):
    if test_d2:
...
if __name__ == '__main__':
    ...
    for idx_file, filedir in enumerate(tqdm(filedir_list)):
        ...
        if args.mode=='lossless':
            ...
            posQuantscaleList = np.array([1, 0.9375, 0.875, 0.75, 0.5, 0.25, 0.125])
            results = test_all(filedir, posQuantscaleList, resolution=args.resolution, test_d2=False)
        print('DBG!!! results:\t', results)
